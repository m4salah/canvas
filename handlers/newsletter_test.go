...
func (s *signupperMock) SignupForNewsletter(
	ctx context.Context,
	email model.Email,
) (string, error) {
	s.email = email
...
func TestNewsletterSignup(t *testing.T) {
	...
	t.Run("signs up a valid email address and send message", func(t *testing.T) {
		is := is.New(t)
...
func (c *confirmerMock) ConfirmNewsletterSignup(
	ctx context.Context,
	token string,
) (*model.Email, error) {
	c.token = token
